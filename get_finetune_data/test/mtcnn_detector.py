...
class MtcnnDetector(object):
    ...
    """
    def __init__(self, model_folder='.', minsize = 20, threshold = [0.6, 0.7, 0.8], factor = 0.709, num_worker = 1, accurate_landmark = False,
                 ctx=mx.cpu()):
        """
    ...
    def detect_face(self, img, det_type=0):
        ...
        if det_type==0:
            ...
            while minl > MIN_DET_SIZE:
                # 这里的scales保存的是12/20*0.709，也就意味着要保存每次进行图像缩小112*0.709之后，分别在每个缩小之后的图像中要检测出的人脸大小。
            ...
            total_boxes = []
            # range(0, 1). ramge(1, 2), range(2,3), range(3, 4), range(4, 5)对应了5种不同的尺度
            ...
            total_boxes = total_boxes[pick]
            #total_boxes的坐标为x1, y1, x2, y2, score, rx1, ry1, rx2, ry2包括四个精修时的偏移坐标
            bbw = total_boxes[:, 2] - total_boxes[:, 0] + 1
            bbh = total_boxes[:, 3] - total_boxes[:, 1] + 1

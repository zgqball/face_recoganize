...
def main(args):
    ...
    dataset = face_image.get_dataset('test', args.input_dir)
    print('dataset size', 'test', len(dataset))
    ...
    property_filename = os.path.join(args.output_dir, 'property')
    with open(property_filename, "w") as text_file:
        oline = '%d,112,112' % (len(dataset))
...
#                 print(img_mask.shape)
                ...
                target_dir = os.path.join(args.output_dir + 'pics/', a)
                if not os.path.exists(args.output_dir + 'pics/'):
                    os.makedirs(args.output_dir + 'pics/')
...
def parse_arguments(argv):
    ...
    parser.add_argument('--input-dir', type=str, help='Directory with unaligned images.',
                        default='../raw_data/train_data/training/')
    parser.add_argument('--output-dir', type=str, help='Directory with aligned face thumbnails.',
                        default='../diff_class_data/')
    parser.add_argument('--bool-mask', type=bool, help='Use mask.', default=True)
